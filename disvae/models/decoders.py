...
class DecoderObjectives(nn.Module):
    def __init__(self, img_size,
        ...
        References:
            ...
            $\beta$-VAE." arXiv preprint arXiv:1804.03599 (2018).
        """
        super(DecoderObjectives, self).__init__()
        ...
        self.batch_norm1d = nn.BatchNorm1d(hidden_dim, hidden_dim)
        self.batch_norm2d = nn.BatchNorm2d(hid_channels)

        self.dropout_objectives = nn.Dropout(p=0.3)
        self.lin3_objectives = nn.Linear(hidden_dim, hidden_dim//2)
        self.lin4_objectives = nn.Linear(hidden_dim//2, hidden_dim//2)
        self.out_objectives = nn.Linear(hidden_dim//2, objectives)
    ...
    def forward(self, z):
        ...
        objectives = torch.relu(self.lin3_objectives(x))
        objectives = self.dropout_objectives(objectives)
        objectives = torch.relu(self.lin4_objectives(objectives))
        objectives = self.out_objectives(objectives)
